...
const MovieCard = ({
  ...
  const [liked, setLiked] = useState(
    isAuthLoading || isFavoritesLoading ? false : isFavorite(movie)
  );
  ...
  const handleHeartClick = () => {
    if (!user)
      console.log(
        "You need to log in to like/unlike a movie. (don't know how you called this funciton btw)"
      );
    if (liked) {
  ...
  return isAuthLoading || isFavoritesLoading ? (
    <Spinner />
  ) : (
    <div className="movie-card">
      ...
      <div className="mt-4">
        ...
        <div className="content">
          ...
          {user && <HeartButton liked={liked} onClick={handleHeartClick} />}
        </div>
      </div>
    </div>
  );
